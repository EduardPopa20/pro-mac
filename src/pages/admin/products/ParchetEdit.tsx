...
const ParchetEdit: React.FC = () => {
  ...
  const [productForm, setProductForm] = useState({
    ...
    // Parchet technical specifications
    thickness: null,
    surface_finish: '',
    texture: '',
    quality_grade: 1,
    ...
    // Parchet capabilities
    is_rectified: false,
    is_frost_resistant: false,
    is_floor_heating_compatible: true, // Most parchet supports floor heating
    
    // Suitability (Parchet is primarily for floors)
    suitable_for_walls: false,
    ...
    if (productSlug && products.length > 0) {
      ...
      if (foundProduct) {
        ...
        setProductForm({
          ...
          plank_width_mm: (foundProduct as any).plank_width_mm || 0,
          plank_length_mm: (foundProduct as any).plank_length_mm || 0,
          installation_type: (foundProduct as any).installation_type || '',
          edge_type: (foundProduct as any).edge_type || '',
          core_material: (foundProduct as any).core_material || '',
          top_layer_thickness: (foundProduct as any).top_layer_thickness || 0,
          ...
          is_floor_heating_compatible: foundProduct.is_floor_heating_compatible !== false, // Default true for parchet
          suitable_for_walls: foundProduct.suitable_for_walls || false,
          suitable_for_floors: foundProduct.suitable_for_floors !== false, // Default true for parchet
  ...
  const handleSave = async () => {
    ...
    try {
      await updateProduct(product.id, {
        ...productForm,
        // Force category-specific constraints for Parchet
        suitable_for_floors: true, // Parchet is always suitable for floors
      ...
      // Navigate back to parchet category page
      navigate('/admin/categorii_produse/parchet')
    } catch (error) {
    ...
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Alert severity="error">
          Produsul nu a fost găsit sau nu este de tip parchet.
        </Alert>
      </Container>
    )
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
        ...
        <Box sx={{ flexGrow: 1 }}>
          ...
          <Typography variant="body1" color="text.secondary">
            Editează proprietățile specifice pentru parchetul: {product.name}
          </Typography>
        </Box>
      ...
      {/* Enhanced Parchet Form - Parchet specific */}
      <EnhancedParchetForm
        productForm={productForm}
        setProductForm={setProductForm}
        categoryType="parchet"
        hideCategories={true} // Hide category selector since this is parchet-specific
